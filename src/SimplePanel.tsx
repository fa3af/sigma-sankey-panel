...
export const SimplePanel: React.FC<Props> = ({
  ...
  if (sourceIpFlag && destinationIpFlag && countFlag) {
    ...
    for (
      let index = 0;
      index < data.series[0].fields[0].values.length;
      index++
    ) {
      ...
      let tooltipText =
        '<div style="color:#000000; padding: 20px 15px; width:100%"><table style="border:none">';
      tooltipText +=
        '<tr><td style="white-space: nowrap;">From</td><td style="white-space: nowrap;"> : <b>' +
        si +
        "</b></td></tr>";
      tooltipText +=
        '<tr><td style="white-space: nowrap;">To</td><td  style="white-space: nowrap;"> : <b>' +
        di +
        "</b></td></tr>";
      tooltipText +=
    ...
    >
      ...
      {!sourceIpFlag && (
        <div className={styles.textBox}>
          Source IP tidak ditemukan, mohon menambahkan source.ip pada group by
          pencarian
        </div>
      )}
      {!destinationIpFlag && (
        <div className={styles.textBox}>
          Destination IP tidak ditemukan, mohon menambahkan destination.ip pada
          group by pencarian
        </div>
      )}
      {!countFlag && (
        <div className={styles.textBox}>
          Metric Count tidak ditemukan, mohon menambahkan Count pada metric
          pencarian
        </div>
      )}
      {/*
